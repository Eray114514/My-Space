...
const FetchModelsModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-xl max-h-[70vh] bg-white dark:bg-[#111] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-white/10"
        onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/5">
          <div>
            <h3 className="text-base font-bold text-gray-900 dark:text-white">{PROVIDER_NAMES[provider] || provider} 模型</h3>
        ...
        <div className="px-5 pt-3">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索模型..."
              className="w-full pl-9 pr-4 py-2 rounded-lg bg-gray-100 dark:bg-white/5 border-0 text-sm outline-none focus:ring-2 focus:ring-[var(--blog-fg)]/20" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-3 space-y-0.5 min-h-0">
          ...
          {!loading && !error && filteredModels.map((m: any) => {
            ...
            return (
              <div key={modelId} className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
        ...
        <div className="px-5 py-3 border-t border-gray-100 dark:border-white/5 flex justify-between items-center">
          <span className="text-xs text-gray-400">{filteredModels.length} 个模型</span>
          <button onClick={onClose} className="px-4 py-1.5 text-sm font-medium rounded-lg bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-colors">
            完成
          </button>
        </div>
      </div>
        ...
        {AI_PROVIDERS.map(p => {
          ...
          return (
            <button key={p.id} onClick={() => setSelectedProvider(p.id)}
              ...
              <Icon size={13} /> {PROVIDER_NAMES[p.id]}
            </button>
          );
        })}
      </div>
              ...
              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); handleToggleFree(key); }}
...
export const ProviderManager: React.FC = () => {
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">获取服务商模型列表并添加，或手动添加</p>
        </div>
        <div className="flex items-center gap-2">
          {AI_PROVIDERS.map(p => (
            <button key={p.id} onClick={() => setFetchModalProvider(p.id)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-colors text-gray-700 dark:text-gray-300">
      ...
      {isAdding && (
        <div className="px-5 py-4 border-b border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-white/[0.02] space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <select value={form.provider} onChange={e => setForm(f => ({ ...f, provider: e.target.value }))}
              className="rounded-lg px-3 py-2 text-sm bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 outline-none focus:ring-2 focus:ring-[var(--blog-fg)]/20">
              {AI_PROVIDERS.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <input value={form.modelId} onChange={e => setForm(f => ({ ...f, modelId: e.target.value }))}
              placeholder="模型 ID (如 deepseek-v4-flash)" className="rounded-lg px-3 py-2 text-sm bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 outline-none focus:ring-2 focus:ring-[var(--blog-fg)]/20" />
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              placeholder="显示名称" className="rounded-lg px-3 py-2 text-sm bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 outline-none focus:ring-2 focus:ring-[var(--blog-fg)]/20" />
            <input value={form.shortName} onChange={e => setForm(f => ({ ...f, shortName: e.target.value }))}
              placeholder="短名称 (可选)" className="rounded-lg px-3 py-2 text-sm bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 outline-none focus:ring-2 focus:ring-[var(--blog-fg)]/20" />
            <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              placeholder="描述 (可选)" className="rounded-lg px-3 py-2 text-sm bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 outline-none focus:ring-2 focus:ring-[var(--blog-fg)]/20 sm:col-span-2" />
          </div>
          ...
          <button onClick={handleAddCustom} className="px-4 py-2 text-xs font-bold rounded-lg bg-[var(--blog-fg)] text-[var(--blog-bg)] hover:opacity-90 transition-opacity">确认添加</button>
        </div>
      )}
      ...
      <div className="divide-y divide-gray-100 dark:divide-white/5">
        {Object.entries(groupedModels).map(([providerId, models]) => {
          ...
          return (
            <div key={providerId}>
              <button onClick={() => setExpandedProvider(isExpanded && Object.keys(groupedModels).length > 1 ? null : providerId)}
                className="w-full flex items-center justify-between px-5 py-3 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-gray-100 dark:bg-white/5 px-1.5 py-0.5 rounded font-medium text-gray-500">{models.length}</span>
                </div>
              ...
              {isExpanded && (
                <div className="px-5 pb-3 space-y-0.5">
                  {models.map(m => (
                    <div key={m.key} className="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors group">
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        <button onClick={() => handleToggleFree(m.key)}
      ...
      {customModels.length === 0 && (
        <div className="text-center py-10 text-sm text-gray-400">暂无模型，点击上方按钮获取或手动添加</div>
      )}
    </div>
  );
};
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 dark:border-white/5">
        ...
        <p className="text-xs text-gray-400 mt-0.5">选择用于生成 SVG 图标的 AI 模型</p>
      </div>
