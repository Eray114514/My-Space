    ...
    <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-xl p-6">
      <div className="mb-4">
        <h4 className="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
          {label}
          <span className="text-xs font-normal text-gray-500 bg-gray-100 dark:bg-neutral-800 px-2 py-0.5 rounded-full">当前: {currentModel?.name || '未设置'}</span>
        </h4>
        ...
        {PROVIDERS.map(p => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors border ${isActive
...
const ProjectEditor: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white/90 dark:bg-neutral-900/90 w-full max-w-lg rounded-2xl shadow-2xl p-6 border border-white/20 dark:border-white/10 max-h-[90vh] overflow-y-auto backdrop-blur-xl">
        <h3 className="text-xl font-bold mb-4 dark:text-white">{project ? '编辑导航' : '新增导航'}</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">标题</label>
            <input required placeholder="网站名称" className="w-full px-3 py-2 bg-white/50 dark:bg-neutral-800/50 border border-gray-200 dark:border-neutral-700 rounded-lg outline-none dark:text-white" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">链接 URL</label>
            <input required type="url" placeholder="https://..." className="w-full px-3 py-2 bg-white/50 dark:bg-neutral-800/50 border border-gray-200 dark:border-neutral-700 rounded-lg outline-none dark:text-white" value={formData.url} onChange={e => setFormData({ ...formData, url: e.target.value })} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">描述</label>
            <textarea placeholder="简短的介绍..." className="w-full px-3 py-2 bg-white/50 dark:bg-neutral-800/50 border border-gray-200 dark:border-neutral-700 rounded-lg outline-none dark:text-white h-20 resize-none" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
          </div>
