...
// 已移除可能不存在或易混淆的图标，确保每一个都能在 Icons 中找到
const CURATED_ICONS = [
...
const ProjectEditor: React.FC<{
    ...
    // 1. Fetch Favicon Logic (Upgraded)
    const handleFetchFavicon = async () => {
        ...
        try {
            ...
            // 使用 Google Favicon V2 API，这个 API 返回更高质量的图标 (通常是 128px)
            // client=SOCIAL 告诉 Google 我们需要类似社交媒体分享的高清图标
            ...
            // Use a CORS proxy to fetch the image data as a Blob
            // 注意：我们必须通过代理才能将图片转为 Base64 存入数据库
            ...
            alert("获取图标失败，该网站可能屏蔽了抓取，或没有高清图标。");
        } finally {
            setFetchingFavicon(false);
        }
    };

    // 2. AI Recommend Icon (Fix "Tool" not found)
    ...
    // 3. AI Generate SVG (Fix Black Screen)
    const handleGenerateSvg = async () => {
        ...
        try {
            ...
            // 关键修复：检查生成的代码是否包含 SVG 标签，防止空字符串或错误信息导致渲染崩溃
            if (svgCode && svgCode.trim().startsWith('<svg')) {
                setFormData(prev => ({ ...prev, customSvg: svgCode, iconType: 'generated' }));
            } else {
                alert("SVG 生成失败：AI 返回了无效的代码，请重试。");
            }
        } catch (e) {
            console.error(e);
            alert("生成出错，请检查 API Key 或网络。");
        } finally {
                    ...
                    <div className="space-y-3 pt-2">
                        ...
                        <div className="flex bg-gray-100 dark:bg-neutral-800 p-1 rounded-lg">
                            <button
                                ...
                                className={`flex-1 flex items-center justify-center gap-2 py-1.5 text-xs font-medium rounded-md transition-all ${formData.iconType === 'auto' ? 'bg-white dark:bg-neutral-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'
                                    }`}
                            >
                                <Globe size={14} /> 自动获取
                            </button>
                            <button
                                ...
                                className={`flex-1 flex items-center justify-center gap-2 py-1.5 text-xs font-medium rounded-md transition-all ${formData.iconType === 'preset' ? 'bg-white dark:bg-neutral-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'
                                    }`}
                            >
                                <Layout size={14} /> 预设图标
                            </button>
                            <button
                                ...
                                className={`flex-1 flex items-center justify-center gap-2 py-1.5 text-xs font-medium rounded-md transition-all ${formData.iconType === 'generated' ? 'bg-white dark:bg-neutral-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'
                                    }`}
                            >
                                <Wand2 size={14} /> AI 生成
                            </button>
                            ...
                            {formData.iconType === 'auto' && (
                                <div className="flex flex-col items-center gap-4 w-full">
                                    ...
                                    >
                                        ...
                                        {fetchingFavicon ? '抓取中...' : '抓取高清图标'}
                                    </button>
                                    <p className="text-[10px] text-gray-400 text-center max-w-xs">
                                        我们将通过代理抓取目标网站的高清图标 (Favicon/Touch Icon) 并将其存入数据库。
                                    </p>
                                </div>
                            )}
                            ...
                            {formData.iconType === 'generated' && (
                                <div className="flex flex-col items-center gap-4 w-full">
                                    <div
                                        ...
                                        // 增加 try-catch 保护，防止无效 SVG 导致页面崩溃 (黑屏)
                ...
                <div className="w-full md:w-1/2 flex flex-col border-b md:border-b-0 md:border-r border-gray-200 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-900/30 p-4 sm:p-6 overflow-y-auto">
                    ...
                    <div className="relative mb-4">
                        ...
                        <div className="absolute right-0 top-0 m-1">
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all shadow-sm border ${isGeneratingSummary
                    ...
                    <div className="flex flex-wrap gap-2 mb-6 items-center">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors ${isAutoTagging
