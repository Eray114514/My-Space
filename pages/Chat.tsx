...
export const Chat: React.FC = () => {
  ...
  // Auto scroll logic
  // Use container.scrollTo instead of element.scrollIntoView to prevent window scrolling (footer reveal)
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto animate-in fade-in duration-500">
      ...
      <div 
        ...
        className="flex-1 overflow-y-auto px-4 sm:px-0 space-y-6 pb-4 scroll-smooth"
      >
      ...
      <div className="mt-4 px-4 sm:px-0 pb-4">
        {/* Changed layout to flex items-end to align button to bottom naturally without absolute positioning issues */}
        <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-2xl shadow-sm focus-within:ring-2 focus-within:ring-indigo-500/50 focus-within:border-indigo-500 transition-all flex items-end p-2 gap-2">
          <textarea
            ref={textareaRef}
            value={input}
            ...
            disabled={isLoading}
            className="flex-1 bg-transparent border-none outline-none px-2 py-3 min-h-[48px] max-h-[150px] resize-none dark:text-white placeholder-gray-400"
            rows={1}
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || isLoading}
            className={`p-2 rounded-xl transition-all shrink-0 mb-1 ${
              !input.trim() || isLoading 
                ? 'text-gray-300 dark:text-neutral-700 bg-gray-100 dark:bg-neutral-800' 
