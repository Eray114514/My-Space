...
export const Chat: React.FC = () => {
  ...
  // Simplify: Store full article objects (or simplified versions) for editing
  ...
  return (
    <div className="flex h-[100dvh] w-full overflow-hidden bg-transparent relative">
        ...
        <div className={`
            ...
            w-72 border-r border-white/10 bg-white/80 dark:bg-black/80 md:bg-white/10 md:dark:bg-black/20 backdrop-blur-xl flex flex-col overflow-hidden
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 custom-scrollbar scroll-smooth">
                {isMessagesLoading ? (
                    ...
                    {messages.map((msg, index) => {
                        ...
                        return (
                            <div key={msg.id} className={`group flex gap-4 max-w-4xl mx-auto ${isUser ? 'flex-row-reverse' : ''} ${isEditing ? 'relative z-50' : 'relative'}`}>
                                        ...
                                        <div className="w-full min-w-[300px] liquid-glass-high rounded-2xl p-2 shadow-xl animate-in fade-in zoom-in-95 duration-200 relative">
                                            ...
                                            {editAttachedArticles.length > 0 && (
                                                <div className="mb-2 flex flex-wrap gap-2">
                                                    {editAttachedArticles.map((art, i) => (
                                                        <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-50/50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 text-xs font-medium text-indigo-600 dark:text-indigo-300">
                                                            <FileText size={12} />
                                                            <span className="truncate max-w-[150px]">{art.title}</span>
                                                            <button onClick={() => setEditAttachedArticles(prev => prev.filter((_, idx) => idx !== i))} className="ml-2 hover:text-red-500 p-0.5 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
                                                                <X size={12} />
                                                            </button>
                                                        </div>
                                                    ))}
                                            ...
                                            <div className="flex justify-between items-center px-2 pb-1 border-t border-black/5 dark:border-white/5 pt-2 relative">
                                                <div className="relative">
                                                     <button onClick={() => setIsEditPickerOpen(!isEditPickerOpen)} className="flex items-center gap-1 px-2 py-1 rounded hover:bg-black/5 dark:hover:bg-white/10 text-xs text-indigo-600 dark:text-indigo-400 font-medium transition-colors">
                                                         <Plus size={12} /> 引用文章
                                                     </button>
                                                     {isEditPickerOpen && (
