...
import { ArrowRight, ExternalLink, Globe, Layout, Code } from 'lucide-react';
...
export const Home: React.FC = () => {
  ...
  const getFaviconUrl = (url: string) => {
    try {
      // Fallback: 使用 Google Favicon V2 API 获取 128px 图标，而不是模糊的 s2/favicons
  ...
  const renderIcon = (project: Project) => {
    ...
    // 2. Saved Base64 Favicon (Priority 1)
    ...
    // 3. Fallback to live high-res URL fetch if 'auto' but no saved image
          ...
          {projects.map((project) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="p-2 bg-gray-50 dark:bg-neutral-800 rounded-xl group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/20 transition-colors overflow-hidden">
                  {renderIcon(project)}
                  {/* Fallback globe for auto mode if image fails */}
                  {project.iconType === 'auto' && <Globe size={32} className="text-gray-300 hidden" />}
                </div>
