...
import { Moon, Sun, Lock, LogOut, Menu, X, LayoutGrid, FileText, Search, MessageSquare, Settings, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'dark' : ''} relative overflow-x-hidden selection:bg-indigo-500/30`}>
      ...
      {!isImmersive && (
        ...
        >
          <header className={`pointer-events-auto liquid-glass-high rounded-full px-3 py-2 flex items-center gap-1 sm:gap-4 shadow-xl transition-all duration-300 ${isSearchPage ? 'w-full max-w-2xl' : 'max-w-full sm:max-w-3xl'}`}>
                ...
                <nav className="hidden md:flex items-center p-1 bg-gray-100/50 dark:bg-white/5 rounded-full border border-black/5 dark:border-white/5">
                  {navLinks.map((link) => (
                    <NavLink
                      ...
                      to={link.path}
                      className={({ isActive }) =>
                        `px-5 py-1.5 rounded-full text-sm font-medium transition-all duration-300 relative ${isActive
